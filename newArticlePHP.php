...
include ("sessions.php");
    ...
    if ($validLog === false) {
        ...
        makePost($connect, $validLog);
        photo($connect, $profile_id, $post_id);
        addTags($connect, $post_id);
        cleanup($connect, $post_id);
    }


    function loginCheck($connect) {
    ...
    function makePost($connect, $chk_user_res){
        global $profile_id; 
        global $post_id;
    ...
    function photo($connect, $profile_id, $post_id){
    ...
    function addTags($connect, $post_id) {
        //Time for tags!====================================================================
        $add_tags_sql = "insert into post_tags_key (post_id, tag_id) values ";
        
        for ($i = 0; $i < $_SESSION['numTags']; $i = $i +1) {
            $clean_tag = mysqli_real_escape_string($connect, $_SESSION['tag' . $i]); 
            $first_let = strtoupper(substr($clean_tag, 0, 1));
            $next_let = strtolower(substr($clean_tag, 1));            
            $clean_tag = $first_let . $next_let;
            ...
            if ($search_tag_res->num_rows < 1) {
                ...
                $new_tag_sql = "insert into post_tags (tag_name) values ('" . 
                    $clean_tag . "')";
    ...
    function cleanup($connect, $post_id) {
        global $post_success;
